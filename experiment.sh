...
python analysis/result_classif.py -idxs `eval echo {1..$RUNS}` -evaldir keras-src/predictions/

echo "##################################"
echo "##################################"

echo "estimated F1 correlation and RMSE"
python analysis/result.py -idxs `eval echo {1..$RUNS}` -prf F1 -evaldir keras-src/predictions/

echo "estimated Precision correlation and RMSE"
python analysis/result.py -idxs `eval echo {1..$RUNS}` -prf P -evaldir keras-src/predictions/

echo "estimated Recall correlation and RMSE"
python analysis/result.py -idxs `eval echo {1..$RUNS}` -prf R -evaldir keras-src/predictions/
